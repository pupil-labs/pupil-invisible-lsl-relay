...
the lsl streaming to the timestamps recorded in cloud. We have build a command line interface
(cli) that allows you to perform a linear model fit, mapping time recorded with lsl to time
in pupil cloud, or vice versa.
...
You can run the command line interface to perform post-hoc time alignment from your terminal.


Start the post-hoc time alignment
=================================
The relay is started by executing::

   lsl_relay_time_alignment <path_to_xdf> <paths_to_exports>

The time alignment requires the following positional arguments:
...
where the info.json file from the pupil cloud export is located. The json file contains three fields:
...
Import the parameters form the json file and use them to transform the linear model
...
As soon as the companion device registers these events, they are streamed to lsl.

In the time alignment, we look for the unique session id in the stream header of the xdf file generated by lsl,
and in the event names available in cloud.
When we find a matching pair, we compute a linear regression (scipy's linear_model.LinearRegression) between the time
stamps of these events in both directions.
