...
The Relay takes five optional arguments:
...
- ``--device_address`` sets the device address explicitly. If you use this tag, the device selection via list
  is skipped. The correct format to pass the device address is ``device_ip:device_port`` the ip address and
  the port of your device can be found in the companion app under Menu -> Streaming

- ``--channel_prefix`` sets the prefix of the channel name displayed in lab recorder. The default prefix
