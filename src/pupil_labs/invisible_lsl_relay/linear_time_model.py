...
def get_filtered_events(xdf_path, cloud_path, xdf_serial_num):
    xdf_event_data = load_xdf_events(
        xdf_path,
        world_camera_serial_num=xdf_serial_num
    )
...
def load_xdf_events(path_to_xdf_recording, world_camera_serial_num):
    stream_type = 'Event'
    data, header = pyxdf.load_xdf(path_to_xdf_recording, select_streams=[
        {'type': stream_type}
    ])

    # need to extract event screen with the correct scene camera
    for event_stream in data:
        if event_stream['info']['desc'][0]['acquisition'][0][
            'world_camera_serial'
        ][0] == world_camera_serial_num:
            return xdf_event_to_df(event_stream)
    raise ValueError(f'None of streams in {path_to_xdf_recording} contained '
                     f'the serial number {world_camera_serial_num}!')


def xdf_event_to_df(event_stream, column_name=event_column_name,
...
def filter_common_events(cloud_data,
                         xdf_data, column_name=event_column_name):

    name_intersection = np.intersect1d(
        cloud_data[column_name], xdf_data[column_name]
    )

    # filter timestamps by the event intersection
    filtered_cloud_data = cloud_data[
        cloud_data[column_name].isin(name_intersection)
    ]
    filtered_xdf_data = xdf_data[
        xdf_data[column_name].isin(name_intersection)
    ]

    return filtered_cloud_data, filtered_xdf_data


def fit_linear_model(filtered_cloud_data,
                     filtered_xdf_data,
                     cloud_path,
                     file_name='gaze.csv',
                     column_timestamp=event_column_timestamp):

    cloud_to_lsl_mapper = linear_time_mapper(filtered_cloud_data[[column_timestamp]],
                                             filtered_xdf_data[column_timestamp])
    lsl_to_cloud_mapper = linear_time_mapper(filtered_xdf_data[[column_timestamp]],
                                             filtered_cloud_data[column_timestamp])
...
def load_df_from_dir(dir_name, file_name, expected_n=1):
    ...
    if len(files) != expected_n:
        raise ValueError(f'There was not the correct number of event files in the '
